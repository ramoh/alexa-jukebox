...
var appServer = function(config) {
  ...
  self.load_apps = function(app_dir, root) {
    ...
    app_directories(app_dir).forEach(function(dir) {
      ...
      if (!fs.existsSync(main) || !fs.statSync(main).isFile()) {
        self.log("   main file not found for app [" + pkg.name + "]: " +
          main);
        return;
      }
      try {
        ...
        if (typeof app.express != "function") {
          self.log("   App [" + pkg.name +
            "] is not an instance of alexa-app");
          return;
        }
        ...
        self.express.post(endpoint, function(req, res) {
          ...
          Promise.resolve(typeof config.preRequest == "function" ?
              config.preRequest(json, req, res) : json)
            ...
            .then(function(app_response_json) {
              ...
              return Promise.resolve(typeof config.postRequest ==
                "function" ? config.postRequest(
                  app_response_json, req, res) :
                app_response_json);
            })
        ...
        if (false !== config.debug) {
          self.express.get(endpoint, function(req, res) {
            ...
            } else if (typeof req.param('utterances') !=
              "undefined") {
        ...
        self.log("   Loaded app [" + pkg.name + "] at endpoint: " +
          endpoint);
      } catch (e) {
  ...
  self.start = function() {
    ...
    var static_dir = path.join(server_root, config.public_html ||
      'public_html');
    if (fs.existsSync(static_dir) && fs.statSync(static_dir).isDirectory()) {
      ...
      self.log("Not serving static content because directory [" +
    ...
    if (fs.existsSync(server_dir) && fs.statSync(server_dir).isDirectory()) {
      ...
      self.log("No server modules loaded because directory [" + server_dir +
        "] does not exist");
    }
    ...
    if (fs.existsSync(app_dir) && fs.statSync(app_dir).isDirectory()) {
      ...
      self.log("Apps not loaded because directory [" + app_dir +
        "] does not exist");
    }
    ...
    if (config.httpsEnabled === true) {
      ...
      if (config.privateKey != undefined && config.certificate != undefined &&
        config.httpsPort != undefined) { //Ensure that all of the needed properties are set
        ...
        if (fs.existsSync(privateKeyFile) && fs.existsSync(certificateFile)) { //Make sure the key and cert exist.
          ...
          if (privateKey != undefined && certificate != undefined) {
            ...
            self.log(
              "Failed to load privateKey or certificate from /sslcert. HTTPS will not be enabled"
            );

          }

        } else {
          self.log("privateKey: '" + config.privateKey +
            "' or certificate: '" + config.certificate +
            "' do not exist in /sslcert. HTTPS will not be enabled");

        }
      } else {
        self.log(
          "privatekey, httpsPort, or certificate paramater not set in config. HTTPS will not be enabled"
        );
